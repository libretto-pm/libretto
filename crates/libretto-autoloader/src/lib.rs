...
"#
    ...
    fn generate_autoload_static(&self, dir: &Path) -> Result<()> {
        ...
        for (char, prefixes) in &prefix_lengths {
            prefix_lengths_php.push_str(&format!("        '{}' => \n", char));
        ...
        for (namespace, paths) in &self.psr4_map {
            ...
            psr4_entries.push_str(&format!("        '{}' => array(\n", escaped_ns));
            for p in paths {
                let relative = self.make_relative_path(p);
                psr4_entries.push_str(&format!("            __DIR__ . '/..' . '{}',\n", relative));
            }
...
"#
    ...
    fn generate_autoload_psr4(&self, dir: &Path) -> Result<()> {
        ...
        for (namespace, paths) in &self.psr4_map {
            let escaped_ns = namespace.replace('\\', "\\\\");
            entries.push_str(&format!("    '{}' => array(\n", escaped_ns));
            for p in paths {
                let relative = self.make_relative_path(p);
                entries.push_str(&format!("        $vendorDir . '{}',\n", relative));
            }
            entries.push_str("    ),\n");
        }
